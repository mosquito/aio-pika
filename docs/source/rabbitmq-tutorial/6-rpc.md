...
:::{warning}
This is a beta version of the port from [official tutorial](https://www.rabbitmq.com/tutorials/tutorial-six-python.html). Please when you found an
...
Since version 1.7.0 [aio-pika](https://github.com/mosquito/aio-pika) has {ref}`patterns submodule <patterns-rpc>`.
...
* (17) As usual we start by establishing the connection and declaring the queue.
* (8) We declare our fibonacci function. It assumes only valid positive integer input.
  (Don't expect this one to work for big numbers, it's probably the slowest recursive implementation possible).
* (31) We declare a callback for basic_consume, the core of the RPC server.
...
* (19) We establish a connection, channel and declare an exclusive 'callback' queue for replies.
* (23) We subscribe to the 'callback' queue, so that we can receive RPC responses.
* (27) The 'on_response' callback executed on every response is doing a very simple job,
...
* (35) Next, we define our main call method - it does the actual RPC request.
* (36) In this method, first we generate a unique correlation_id number and save it - the 'on_response' callback
  function will use this value to catch the appropriate response.
* (42) Next, we publish the request message, with two properties: reply_to and correlation_id.
...
:::{note}
This material was adopted from [official tutorial](https://www.rabbitmq.com/tutorials/tutorial-six-python.html) on **rabbitmq.org**.
:::
